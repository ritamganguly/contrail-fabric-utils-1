...
def role_to_ip_dict_utility(role = None):
    role_to_ip_dict = {}
    for each_key in env.roledefs:
        role_to_ip_dict[each_key] = [hstr_to_ip(get_control_host_string(each_host)) for each_host in env.roledefs[each_key]]
    if role != None:
            ...
            try:
                if env.rsyslog_params['status'].lower() == 'enable':
                    try:
                        ...
                        rsyslog_port = def_port #Hard codded default port number.
                    try:
                        env.rsyslog_params['proto']
                        rsyslog_proto = env.rsyslog_params['proto']
                    except:
                        rsyslog_proto = def_proto #Hard codded default port number.
                    with cd(UTILS_DIR):
